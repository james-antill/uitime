...
func parseEpoch(datetime string) (int64, int64, error) {
    switch {
    ...
    return 0, 0, fmt.Errorf("Not epoch number")
}

func ptime(now time.Time, datetime string) time.Time {
    // 0. Try an Epoch
    if epochs, epochns, err := parseEpoch(datetime); err == nil {
        return time.Unix(epochs, epochns)
    }
